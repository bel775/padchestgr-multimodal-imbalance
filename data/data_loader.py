...
from dataset import CustomDataset, CachedFeatureDataset
...
def get_data(data_grouped,mlb, training_mode, images_src,RadDino_src,RadDinoWeights, IMAGE_SIZE = 448, batch_size = 32, 
    ...
    dataset_train = CustomDataset(data_grouped, training_mode, images_src, IMAGE_SIZE=IMAGE_SIZE, split="train", DataAug = DataAug)
    dataset_val   = CustomDataset(data_grouped, training_mode, images_src, IMAGE_SIZE=IMAGE_SIZE, split="val", DataAug = DataAug)
    dataset_test  = CustomDataset(data_grouped, training_mode, images_src, IMAGE_SIZE=IMAGE_SIZE, split="test", DataAug = DataAug)
    ...
    if oversampler:
        ...
        if feature_extract_image:
            train_dataset_extractedFeatures, _ = extract_features_rad_dino(train_dataset,training_mode, image_encoder)
            ...
            train_dataset = CachedFeatureDataset(train_dataset_feats)
    ...
    if feature_extract_image:
        val_dataset_extractedFeatures, _ = extract_features_rad_dino(val_dataset, training_mode, image_encoder)
        test_dataset_extractedFeatures, feat_dim = extract_features_rad_dino(test_dataset, training_mode, image_encoder)
        ...
        val_dataset = CachedFeatureDataset(val_dataset_feats)
        test_dataset_feats, fusion_dim = extract_text_features(test_dataset, TextFeaturesextract_model)
        test_dataset = CachedFeatureDataset(test_dataset_feats)
